...
All water temperature logger sites were visited in Spring / Summer 2023, and log files were downloaded from each logger. The process QA/QC described from Fall 2022 was applied in an identical fashion. QA/QC files are available in the project's GitHub repository.
...
# note: long term KWF site at beaver creek did not have data available from logger #20012591 in summer 2023. We will instead use paired logger #21488142


# 7/27/23 - missing ONE logger files from summer 2023 - may be on lab rugged notebook desktop
...
(source csv files from diff folder and take through same process. remove dates from prior to earliest previous round of visits, join two dfs, then exclude redundant rows. hide code chunks since we already showed it once)
...
<REDO THIS SECTION INTEGRATE NEW DATA FROM 2023>
